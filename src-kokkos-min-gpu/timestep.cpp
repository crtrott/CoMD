...
{
   Kokkos::parallel_for(Kokkos::TeamPolicy<>(nBoxes,TEAM_SIZE), KOKKOS_LAMBDA(const TeamType& team) 
   {
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team,s->boxes->nAtoms[team.league_rank()]), [&] (const int ii) 
      {
         const int iOff = MAXATOMS*team.league_rank() + ii; 
...
void kineticEnergy(SimFlat* s)
{
   ...
   eLocal[1] = 0;
   Kokkos::parallel_reduce(Kokkos::TeamPolicy<>(s->boxes->nLocalBoxes,TEAM_SIZE), KOKKOS_LAMBDA(const TeamType& team,real_t& kenergy_team) 
   {
      real_t kenergy_tmp = 0.0;
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team,s->boxes->nAtoms[team.league_rank()]), [&] (const int ii, real_t& kenergy) 
      {
         const int iOff = MAXATOMS*team.league_rank() + ii; 
         int iSpecies = s->atoms->iSpecies[iOff];
...
void redistributeAtoms(SimFlat* sim)
{
   ...
   Kokkos::parallel_for(sim->boxes->nTotalBoxes, KOKKOS_LAMBDA(const int& ii) {
