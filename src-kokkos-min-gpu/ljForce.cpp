...
void ljPrint(FILE* file, BasePotential* pot)
{
   ...
   fprintf(file, "  Epsilon          : %lf eV\n", ljPot->epsilon);
   fprintf(file, "  Sigma            : %lf Angstroms\n", ljPot->sigma);
}
...
int ljForce(SimFlat* s)
{
   ...
   Kokkos::parallel_reduce(Kokkos::TeamPolicy<>(s->boxes->nTotalBoxes,TEAM_VECTOR_SIZE,VECTOR_SIZE), KOKKOS_LAMBDA(const TeamType& team, double& ePot_team)
   {
      ...
      double ePot_tmp;
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team,nNbrBoxes), [&] (const int& jTmp, double& ePot_thread)  
      {
         ...
         Kokkos::parallel_reduce(Kokkos::ThreadVectorRange(team, nIBox), [&] (int iOff, double& ePot)
         {
            iOff += MAXATOMS*iBox;
         ...
         },ePot_thread); // loop over atoms in iBox
      }, ePot_tmp); // loop over neighbor boxes
