...
Atoms* initAtoms(LinkCell* boxes)
{
   ...
   Kokkos::parallel_for(maxTotalAtoms, [&] (const int& iOff)
   {
      atoms->gid[iOff] = 0;
...
void setVcm(SimFlat* s, real_t newVcm[3])
{
   ...
   Kokkos::parallel_for(Kokkos::TeamPolicy<>(s->boxes->nLocalBoxes,TEAM_SIZE), KOKKOS_LAMBDA (const TeamType& team)
   {
      const int iBox = team.league_rank();
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team,s->boxes->nAtoms[iBox]), [&] (const int& ii) 
      {
         const int iOff = MAXATOMS*iBox + ii;
