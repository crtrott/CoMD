...
int eamForce(SimFlat* s)
{
   ...
   Kokkos::parallel_for(fsize,KOKKOS_LAMBDA (const int& ii) 
   {
   ...
   Kokkos::parallel_reduce(Kokkos::TeamPolicy<>(s->boxes->nTotalBoxes,TEAM_VECTOR_SIZE,VECTOR_SIZE), KOKKOS_LAMBDA(const TeamType& team, double& etot_team)
   {
      ...
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team,nNbrBoxes), [&] (const int& jTmp, double& etot_thread) 
      {
         int jBox = s->boxes->nbrBoxes[iBox][jTmp];
         int nJBox = s->boxes->nAtoms[jBox];

         // loop over atoms in iBox
         Kokkos::parallel_reduce(Kokkos::ThreadVectorRange(team,nIBox), [&] (int iOff, double& etot) 
         {
            iOff += MAXATOMS*iBox;
         ...
         },etot_thread); // loop over atoms in iBox
      },etot_tmp); // loop over neighbor boxes
      Kokkos::single(Kokkos::PerTeam(team), [&] () {
        etot_team += etot_tmp;
      });
   },etot); // loop over local boxes

   double etot2 = 0.0;
   // Compute Embedding Energy
   // loop over all local boxes
   Kokkos::parallel_reduce(Kokkos::TeamPolicy<>(s->boxes->nTotalBoxes,TEAM_SIZE), KOKKOS_LAMBDA(const TeamType& team, double& etot_team)
   {
      const int iBox = team.league_rank();
      int nIBox =  s->boxes->nAtoms[iBox];
      double etot_tmp;
      // loop over atoms in iBox
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team,MAXATOMS*iBox,MAXATOMS*iBox+nIBox), [&] (int iOff, double& etot_thread) 
      {
   ...
   Kokkos::parallel_for(Kokkos::TeamPolicy<>(s->boxes->nTotalBoxes,TEAM_VECTOR_SIZE,VECTOR_SIZE), KOKKOS_LAMBDA(const TeamType& team)
   {
      ...
      // loop over neighbor boxes of iBox (some may be halo boxes)
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team,nNbrBoxes), [&](const int& jTmp) 
      {
         int jBox = s->boxes->nbrBoxes[iBox][jTmp];
         int nJBox = s->boxes->nAtoms[jBox];

         // loop over atoms in iBox
         Kokkos::parallel_for(Kokkos::ThreadVectorRange(team,nIBox), [&] (int iOff)
         {
            iOff += MAXATOMS*iBox;
            // loop over atoms in jBox
...
void eamPrint(FILE* file, BasePotential* pot)
{
   ...
   fprintf(file, "  Mass            : %lf amu\n", eamPot->mass/amuToInternalMass); // print in amu
   fprintf(file, "  Lattice type    : %s\n", eamPot->latticeType);
   fprintf(file, "  Lattice spacing : %lf Angstroms\n", eamPot->lat);
   fprintf(file, "  Cutoff          : %lf Angstroms\n", eamPot->cutoff);
}
   ...
   int n, real_t x0, real_t dx, real_t* data)
{
   InterpolationObject* table =
      comdMalloc<InterpolationObject>(1) ;
   assert(table);
